...
const dailyVerses = [
  {
    de: {
      text: "„Wir wissen aber, dass denen, die Gott lieben, alle Dinge zum Besten dienen.“",
      ref: "Römer 8:28",
    },
    en: {
  ...
  {
    de: {
      text: "„Alle eure Dinge lasst in der Liebe geschehen!“",
      ref: "1. Korinther 16:14",
    },
  ...
  {
    de: {
      text: "„Befiehl dem Herrn deine Wege und hoffe auf ihn, er wird's wohlmachen.“",
      ref: "Psalm 37:5",
    },
    en: {
  ...
  {
    de: {
      text: "„Gott ist unsere Zuversicht und Stärke, eine Hilfe in den großen Nöten.“",
      ref: "Psalm 46:2",
    },
    en: {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- DYNAMISCHES KARUSSELL & LAZY LOADING ---
  const carousel = document.getElementById("featured-carousel");
  const observerTarget = document.getElementById("carousel-observer-target");
  ...
  async function loadRandomRecipes() {
    if (isLoading || !carousel) return;
    isLoading = true;

    try {
      const excludeParam =
        loadedRecipeIds.length > 0
          ? `&exclude=${loadedRecipeIds.join(",")}`
          : "";
      const response = await fetch(
        `https://recipe777-test.loca.lt/`,
      );
      ...
      recipes.forEach((recipe) => {
        ...
        card.className = "recipe-card snap-card";

        card.innerHTML = `
          <div class="card-image-wrapper">
            <a href="pages/recipe-detail.html?id=${recipe.id}" style="display: block; text-decoration: none;">
              <img src="${recipe.imageUrl || "assets/images/torte.webp"}" alt="${recipe.title}" class="card-img" loading="lazy" />
          ...
          <div class="card-details">
            <div class="details-content">
              <p><strong>${text.timeLbl}</strong> ${recipe.prepTime} Min.</p>
              <ul>
        ...
        carousel.insertBefore(card, observerTarget);
      });
    } catch (err) {
  ...
  // Observer für Infinite Scroll (Lazy Loading)
  if (carousel && observerTarget) {
      ...
      if (e.target.classList.contains("expand-btn")) {
        ...
        setTimeout(() => {
          if (isOpen) {
            ...
            btn.textContent = text.btnCollapse;
          }
          btn.style.opacity = "1";
        }, 150);
      }
